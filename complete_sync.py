...
def run_command(
    command: List[str], check: bool = True, cwd: Optional[str] = None
) -> Tuple[bool, str]:
    """Run command and return result with enhanced error handling"""
    print(f"🔧 Running: {' '.join(command)}")
    try:
        result = subprocess.run(
            command, capture_output=True, text=True, check=check, cwd=cwd or os.getcwd()
        )
...
def sync_with_remote(git_info: Dict[str, any]) -> bool:
    ...
    success, behind_output = run_command(
        ["git", "rev-list", "--count", "HEAD..origin/main"], check=False
    )
...
def generate_sync_report(
    validation_results: Dict, git_info: Dict, backup_path: Optional[str]
) -> None:
    ...
    total_missing = len(validation_results.get("missing_files", [])) + len(
        validation_results.get("missing_dirs", [])
    )
    total_present = len(validation_results.get("present", []))

    print(
        f"📊 Project Structure: {total_present} items present, {total_missing} missing"
    )
    print(f"🔗 Git Repository: {git_info.get('current_branch', 'unknown')} branch")
    print(
        f"🌐 Remote Access: {'✅' if git_info.get('remote_accessible', False) else '❌'}"
    )
