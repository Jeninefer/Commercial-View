...
def get_loan_data():
    try:
        return load_loan_data(PRICING_BASE_PATH).to_dict(orient="records")
...
def get_historic_real_payment():
    try:
        return load_historic_real_payment(PRICING_BASE_PATH).to_dict(orient="records")
    except FileNotFoundError:
...
def get_payment_schedule():
    try:
        return load_payment_schedule(PRICING_BASE_PATH).to_dict(orient="records")
    except FileNotFoundError:
...
def get_customer_data():
    try:
        return load_customer_data(PRICING_BASE_PATH).to_dict(orient="records")
    except FileNotFoundError:
...
def get_collateral():
    try:
        return load_collateral(PRICING_BASE_PATH).to_dict(orient="records")
    except FileNotFoundError:
