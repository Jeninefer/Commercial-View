...
def create_coverage_summary_report(analysis: Dict, reports: Dict) -> None:
    ...
    with open(summary_file, 'w') as f:
        ...
        low_coverage_modules = [
            module for module, data in analysis.get("module_coverage", {}).items()
            if data["average"] < 70
        ]
...
def run_coverage_analysis():
    ...
    if "total_coverage" in analysis:
        ...
        for module, data in analysis.get("module_coverage", {}).items():
            status = "✅" if data["average"] >= 80 else "⚠️" if data["average"] >= 60 else "❌"
            print(f"   {status} {module}: {data['average']:.1f}% ({data['file_count']} files)")
