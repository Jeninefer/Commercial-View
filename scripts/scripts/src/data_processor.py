...
class DataProcessor:
    ...
    def convert_dates_safely(
        self, df: pd.DataFrame, date_columns: list
    ) -> pd.DataFrame:
        ...
        for col in date_columns:
            if col in result_df.columns:
                result_df[col] = pd.to_datetime(
                    result_df[col], errors="coerce", infer_datetime_format=True
                )
    ...
    def process_loan_schedules(self, schedule_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ...existing code...
        processed_df = self.convert_dates_safely(
            schedule_df, ["payment_date", "due_date"]
        )
