...
class ProductionDatasetGenerator:
    ...
    def _generate_loan_data(self) -> str:
        ...
        for i in range(num_loans):
            if i < num_customers:
                ...
                customer_ids.append(f"CUST{str(random.randint(1, num_customers)).zfill(4)}")
        ...
        for amount in loan_amounts:
            ...
            size_adjustment = -0.02 if amount > 1000000 else 0.01  # Large loans get better rates
        ...
        terms = np.random.choice([12, 24, 36, 48, 60, 72], num_loans, 
        ...
        industry_codes = np.random.choice([
            "236220", "238210", "541511", "722513", "445120",
            "238990", "531210", "424710", "484110", "561720"
        ], num_loans)
        ...
        loan_df = pd.DataFrame({
            ...
            "remaining_balance": [round(amt * random.uniform(0.2, 0.95), 2) for amt in loan_amounts]
        })
        ...
        for _, loan in loan_df.iterrows():
            ...
            for month in range(term):
                ...
                payment_schedules.append({
                    ...
                    "payment_status": "pending" if payment_date > datetime.now() else 
                                   np.random.choice(["paid", "overdue", "partial"], p=[0.85, 0.10, 0.05])
                })
    ...
    def _generate_historic_payments(self) -> str:
        ...
        logger.info(f"✅ Generated {filename}: {len(historic_df)} historical payment records")
        return str(filepath)
    ...
    def _generate_customer_data(self) -> str:
        ...
        last_names = ["Smith", "Doe", "Brown", "Johnson", "Lee", "Wilson", "Moore", "Taylor"]
        ...
        phone_numbers = [f"+1-800-{random.randint(100, 999)}-{random.randint(1000, 9999)}" for _ in range(num_customers)]
        
        # Addresses (dummy data)
        addresses = [f"{random.randint(100, 999)} {' '.join(random.choices(['Main St', '2nd St', '3rd St', '4th St'], k=1))}, Cityville, ST, {random.randint(10000, 99999)}" for _ in range(num_customers)]
        
        # Demographics
        ages = np.random.randint(18, 70, num_customers)
        incomes = np.random.randint(30000, 120000, num_customers)
        employment_status = np.random.choice(["employed", "unemployed", "self-employed", "retired"], num_customers, p=[0.6, 0.2, 0.15, 0.05])
    ...
    def _generate_collateral_data(self) -> str:
        ...
        # Collateral types
        collateral_types = np.random.choice(["real_estate", "vehicle", "equipment", "inventory"], num_collaterals)
