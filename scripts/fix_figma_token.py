...
def test_commercial_view_access(
    token: str, headers: Dict, user_info: Dict
) -> Tuple[bool, Dict]:
...
def test_dashboard_components(
    token: str, headers: Dict, file_id: str
) -> Tuple[bool, Dict]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for comp_id, comp_info in (
                ...
                if any(
                    keyword in comp_info.get("name", "").lower()
                    for keyword in [
                        "kpi",
                        "dashboard",
                        "metric",
...
def find_token_references() -> List[Path]:
    ...
    search_patterns = ["**/*.json", "**/*.py", "**/*.md", "**/.env*", "**/config*"]
