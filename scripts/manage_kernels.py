...
class CommercialViewKernelManager:
    ...
    def create_kernel_spec(self) -> Dict:
        ...
        return {
            ...
            "env": {
                ...
                "PRICING_CONFIG_PATH": str(self.project_root / "configs" / "pricing_config.yml"),
                "DPD_POLICY_PATH": str(self.project_root / "configs" / "dpd_policy.yml"),
                "COLUMN_MAPS_PATH": str(self.project_root / "configs" / "column_maps.yml"),
    ...
    def install_kernel(self, force: bool = False) -> bool:
        ...
        try:
            ...
            python_exe = self.venv_path / "bin" / "python"
            cmd = [
                str(python_exe), "-m", "jupyter", "kernelspec", "install",
   ...
   Commercial Lending Analytics
    ...
    def uninstall_kernel(self) -> bool:
        ...
        try:
            python_exe = self.venv_path / "bin" / "python"
            cmd = [
                str(python_exe), "-m", "jupyter", "kernelspec", "remove",
                self.kernel_name, "-f"
            ]
...
Examples:
  ...
  %(prog)s setup-extensions     # Install Jupyter extensions
    ...
    parser.add_argument("--force", action="store_true",
                       help="Force reinstall (for install action)")
