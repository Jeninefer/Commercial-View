...
class UvicornManager:
    ...
    def setup_logging(self, log_level: str = "info") -> None:
        ...
        log_config = {
            ...
            "loggers": {
                "uvicorn": {"level": log_level.upper(), "handlers": ["file", "console"]},
    ...
    def start_server(
        ...
        cmd = [
            ...
            "--port", str(port),
            "--log-level", log_level,
            "--log-config", str(self.create_log_config(log_level))
        ]
        ...
        if access_log:
            cmd.extend(["--access-log", "--access-log-format", 
        ...
        cmd.extend([
            ...
            "--lifespan", "on",  # Enable lifespan events for initialization
    ...
    def check_server_health(self, host: str = "localhost", port: int = 8000, ssl: bool = False) -> Dict[str, Any]:
                ...
                for endpoint in endpoints_to_check:
                    try:
                        ...
                        health_status["api_endpoints"][endpoint] = {
                            ...
                            "available": ep_response.status_code in [200, 404, 405]  # 404/405 means endpoint exists but method not allowed
                        }
                    except Exception as e:
                        health_status["api_endpoints"][endpoint] = {
                            "status_code": None,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Manage Commercial-View Commercial Lending Server")
    parser.add_argument("action", choices=["dev", "prod", "perf", "kill", "health", "status"],
    ...
    if args.action == "dev":
        ...
        if args.json:
            ...
            print(f"🏦 Commercial-View Status: {status['commercial_view_status'].upper()}")
            if status["process_running"]:
                print(f"📊 CPU: {status['resource_usage']['cpu_percent']}%")
                print(f"💾 Memory: {status['resource_usage']['memory_mb']} MB ({status['resource_usage']['memory_percent']:.1f}%)")
