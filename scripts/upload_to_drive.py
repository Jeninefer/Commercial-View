...
try:
    ...
    class MockCredentials:
        ...
        @staticmethod
        def from_authorized_user_file(*args):
            return None
    ...
    class MockInstalledAppFlow:
        @staticmethod
        def from_client_secrets_file(*args):
            return None
...
def check_dependencies() -> bool:
    ...
    if not GOOGLE_DRIVE_AVAILABLE:
        ...
        try:
            response = (
                input("   Install dependencies automatically? (y/n): ").lower().strip()
            )
            if response == "y":
...
class DriveUploader:
        ...
        for attempt in range(max_retries):
            try:
                    ...
                    try:
                        ...
                        if status:
                            ...
                            if progress - last_progress >= 10:  # Update every 10%
                                ...
                                speed = (
                                    (file_size * status.progress())
                                    / (1024 * 1024 * elapsed)
                                    if elapsed > 0
                                    else 0
                                )
                                print(
                                    f"📤 Uploading {filename}: {progress}% ({speed:.1f} MB/s)"
                                )
                                last_progress = progress
                    except Exception as e:
                        if attempt < max_retries - 1:
                            print(
                                f"⚠️  Upload interrupted, retrying... (attempt {attempt + 1}/{max_retries})"
                            )
                ...
                if response:
                    ...
                    avg_speed = (
                        (file_size / (1024 * 1024)) / upload_time
                        if upload_time > 0
                        else 0
                    )
                ...
                if attempt < max_retries - 1:
                    ...
                    print(
                        f"❌ Failed to upload {filename} after {max_retries} attempts: {e}"
                    )
                    return False
    ...
    def get_upload_statistics(self) -> Dict[str, Any]:
        ...
        total_time = sum(
            upload["upload_time_seconds"] for upload in self.upload_history
        )
        avg_speed = (
            sum(upload["avg_speed_mbps"] for upload in self.upload_history)
            / total_files
        )
        ...
        return {
            ...
            "files_per_minute": (
                (total_files / (total_time / 60)) if total_time > 0 else 0
            ),
        }
...
def create_upload_summary(
    uploader: DriveUploader, config: Dict, success_count: int, total_files: int
) -> None:
...
def perform_file_uploads(
    uploader: DriveUploader, files_to_upload: list, target_folder_id: str, config: Dict
) -> int:
    """Perform the actual file uploads with enhanced progress tracking"""
    success_count = 0
    total_files = len(
        [f for f in files_to_upload if f[0] in config.get("upload_types", [])]
    )
    current_file = 0
...
def upload_exports_to_drive(
    ...
    if dry_run:
        ...
        print(
            f"🔗 Target folder: https://drive.google.com/drive/folders/{config['drive_folder_id']}"
        )
        return True
    ...
    # Perform uploads
    success_count = perform_file_uploads(
        uploader, files_to_upload, target_folder_id, config
    )
...
Examples:
    ...
    parser.add_argument("--folder-id", help="Google Drive folder ID (overrides config)")
    ...
    success = upload_exports_to_drive(
        export_dir=args.export_dir, config=config, dry_run=args.dry_run
    )
