...
def check_git_health() -> Dict[str, str]:
    ...
    try:
        ahead_behind = run_git_command(["git", "rev-list", "--left-right", "--count", "HEAD...origin/main"])
...
def analyze_project_health() -> Dict[str, any]:
    ...
    health_metrics = {
        ...
        "frontend_components": len(glob.glob("frontend/**/*.js", recursive=True)) + len(glob.glob("frontend/**/*.jsx", recursive=True)),
...
def generate_sync_report(project_structure: Dict, git_info: Dict, health_metrics: Dict) -> None:
    """Generate comprehensive synchronization report"""
    print(f"\n📋 Synchronization Report")
    print("=" * 50)
    
    # Calculate missing files
    total_missing = sum(len(category["missing"]) for category in project_structure.values())
    total_files = sum(len(category["present"]) + len(category["missing"]) for category in project_structure.values())
    
    completion_rate = ((total_files - total_missing) / total_files * 100) if total_files > 0 else 0
    
    print(f"📈 Project Completion: {completion_rate:.1f}% ({total_files - total_missing}/{total_files} files)")
    print(f"🔍 Repository Status: {'✅ Clean' if not git_info.get('has_changes', True) else '⚠️  Has Changes'}")
    ...
    if total_missing > 0:
        ...
        for category, files in project_structure.items():
            if files["missing"]:
                print(f"  📁 {category.replace('_', ' ').title()}: {len(files['missing'])} missing")
