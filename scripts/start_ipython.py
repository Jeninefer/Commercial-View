...
def create_commercial_view_namespace() -> Dict[str, Any]:
        ...
        if sample_files:
            ...
            if user_ns.get('pd') is not None:
                try:
                    ...
                    print(f"📈 Loaded {sample_files[0].name}: {df.shape[0]} rows, {df.shape[1]} columns")
                    return df
                except Exception as e:
        ...
        for name, config in configs.items():
            if config:
                ...
                if isinstance(config, dict) and name == 'figma':
                    dashboard_id = config.get('figma', {}).get('commercial_view', {}).get('dashboard_file_id')
                    if dashboard_id:
    ...
    def figma_status():
        ...
        if figma_config:
            ...
            dashboard_id = figma_config.get('figma', {}).get('commercial_view', {}).get('dashboard_file_id')
            if dashboard_id:
...
🚀 Quick start commands:
  ...
  %autoreload 2       - Auto-reload modules on change
        ...
        print("💡 For full development environment: pip install ipython pandas numpy matplotlib seaborn")
        sys.exit(1)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "--config":
            ...
            setup_commercial_view_environment()
            user_ns = create_commercial_view_namespace()
            available_modules = len([k for k, v in user_ns.items() if v is not None and not k.startswith('_')])
