...
class CommercialViewProductionValidator:
    ...
    def _validate_english_compliance(self):
        ...
        non_english_patterns = [
            ...
            (r'\b(español|français|deutsch|italiano|português|中文|日本語)\b', 'Non-English language detected'),
    ...
    def _validate_no_demo_data(self):
        ...
        for file_path in self.repo_root.rglob("*"):
            if file_path.is_file():
                ...
                if any(term in filename for term in ['demo', 'example', 'sample', 'mock', 'test_']):
    ...
    def _validate_production_readiness(self):
        ...
        google_drive_url = "https://drive.google.com/drive/folders/1qIg_BnIf_IWYcWqCuvLaYU_Gu4C2-Dj8"
    ...
    def _get_text_files(self) -> List[Path]:
        ...
        text_extensions = {'.py', '.md', '.txt', '.json', '.yml', '.yaml', '.csv', '.ts', '.tsx', '.js', '.jsx'}
    ...
    def _generate_final_assessment(self):
        ...
        self.validation_results["overall_status"] = "PRODUCTION_READY" if all_passed else "NEEDS_FIXES"
        ...
        print()
        print(f"Overall Status: {'🎉 PRODUCTION READY' if all_passed else '⚠️ NEEDS FIXES'}")
        ...
        if all_passed:
            ...
            for category, data in self.validation_results.items():
                if isinstance(data, dict) and not data.get("status", True) and "issues" in data:
                    print(f"\n{category.replace('_', ' ').title()}:")
                    for issue in data["issues"][:3]:  # Show first 3 issues
                        if isinstance(issue, dict):
                            print(f"  - {issue.get('file', 'Unknown')}: {issue.get('issue', 'Unknown issue')}")
