...
class ProductionDataValidator:
        ...
        for csv_file in csv_files:
            try:
                ...
                validation_results["issues_found"].append(
                    {"file": csv_file.name, "issue": f"Failed to load: {e}"}
                )

        validation_results["all_production_ready"] = (
            validation_results["production_ready"]
            == validation_results["datasets_validated"]
            and len(validation_results["issues_found"]) == 0
        )
    ...
    def _validate_dataset(self, df: pd.DataFrame, filename: str) -> Dict[str, any]:
        ...
        for col in string_columns:
            for demo_flag in self.demo_data_flags:
                if (
                    ...
                    .astype(str)
                    .str.lower()
                    .str.contains(demo_flag, na=False)
                    .any()
                ):
                    issues.append(
                        {
        ...
        for col in numeric_columns:
            ...
            if col.lower() in ["amount", "balance", "principal"]:
                round_numbers = (
                    df[col]
                    .apply(lambda x: x % 1000 == 0 if pd.notna(x) else False)
                    .sum()
                )
        ...
        return {
            "production_ready": len([i for i in issues if i["severity"] == "high"])
            == 0,
            "issues": issues,
            "record_count": len(df),
    ...
    def _validate_commercial_loan_data(self, df: pd.DataFrame, issues: List) -> bool:
        ...
        amount_cols = [
            ...
            if "amount" in col.lower() or "principal" in col.lower()
        ]
        ...
        rate_cols = [
            col for col in df.columns if "rate" in col.lower() or "apr" in col.lower()
        ]
