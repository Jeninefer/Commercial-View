...
class TestDataLoader:
    ...
    def test_load_historic_real_payment(self, temp_data_dir):
        ...
        df = load_historic_real_payment(
            temp_data_dir
        )  # Changed from base_path= to positional
        assert not df.empty
        assert len(df) == 2

    def test_load_payment_schedule(self, temp_data_dir):
        """Test loading payment schedule data."""
        df = load_payment_schedule(
            temp_data_dir
        )  # Changed from base_path= to positional
        assert not df.empty
        assert len(df) == 2
    ...
    def test_direct_file_path(self, temp_data_dir):
        ...
        df = pd.DataFrame({"Customer ID": ["C001", "C002"], "Amount": [1000, 2000]})
    ...
    class TestDataLoader:
        ...
        def test_data_validation_missing_columns(self, temp_data_dir):
            ...
            invalid_data = pd.DataFrame({"Invalid Column": ["value1", "value2"]})
        ...
        def test_environment_variable_priority(self, temp_data_dir):
            ...
            with patch.dict(
                os.environ, {"COMMERCIAL_VIEW_DATA_PATH": "/different/path"}
            ):
