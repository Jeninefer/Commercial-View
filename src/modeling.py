...
REQUIRED_PD_FEATURES = ["outstanding_principal", "interest_rate", "loan_amount", "dpd"]
...
class PDModel:
    ...
    def _validate_training_data(
        self, loan_df: pd.DataFrame, target_column: str
    ) -> Tuple[bool, str]:
        """
        ...
        if len(loan_df) < MIN_TRAINING_SAMPLES:
            return (
                False,
                f"Insufficient training samples (minimum: {MIN_TRAINING_SAMPLES})",
            )
...
class ChurnModel:
    ...
    def train(
        self, customer_df: pd.DataFrame, target_column: str = "churned"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Split data
            X_train, X_test, y_train, y_test = train_test_split(
                X, y, test_size=DEFAULT_TEST_SIZE, random_state=self.random_state
            )
            ...
            self.is_trained = True
            logger.info(
                f"✅ Churn model trained successfully (AUC: {metrics['roc_auc']:.3f})"
            )
...
def run_stress_test(
    ...
    try:
        ...
        loss_increase_pct = (
            (incremental_loss / baseline_loss * 100.0) if baseline_loss > 0 else 0.0
        )
