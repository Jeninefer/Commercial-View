...
def update_snapshot(
    snap: Dict[str, Any], new_money: Optional[float] = None
) -> Dict[str, Any]:
    ...
    if (
        ...
        and "valuation" in snap
        and snap["valuation"].get("pre_money")
    ):
...
class KPICalculator:
    ...
    def calculate_portfolio_yield(self, loan_df: pd.DataFrame) -> float:
        ...
        if (
            "interest_income" in loan_df.columns
            and "outstanding_balance" in loan_df.columns
        ):
    ...
    def calculate_collection_efficiency(self, payment_df: pd.DataFrame) -> float:
        ...
        if (
            "collected_amount" in payment_df.columns
            and "scheduled_amount" in payment_df.columns
        ):
