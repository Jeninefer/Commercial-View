...
class AbacoCore:
    ...
    def standardize_data_formats(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        date_columns = ['disbursement_date', 'payment_date', 'maturity_date', 'last_active_date']
            ...
            if col in result_df.columns:
                try:
                    ...
                    if col == 'loan_amount':
                        ...
                        invalid_amounts = ((result_df[col] < min_amount) | 
                                         (result_df[col] > max_amount)).sum()
                        if invalid_amounts > 0:
                            logger.warning(f"Found {invalid_amounts} invalid loan amounts")
                except Exception as e:
    ...
    def validate_data_quality(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if missing_cells > 0:
                quality_report['recommendations'].append(f"Address {missing_cells} missing values")
            if quality_report['duplicate_records'] > 0:
                quality_report['recommendations'].append(f"Remove {quality_report['duplicate_records']} duplicate records")
            if quality_score < 80:
                quality_report['recommendations'].append("Data quality below 80% - review data sources")
    ...
    def create_data_snapshot(self, df: pd.DataFrame, snapshot_name: str) -> Dict[str, Any]:
