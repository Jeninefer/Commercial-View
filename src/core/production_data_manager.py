...
class ProductionDataManager:
    ...
    def __init__(self):
        ...
        for directory in [self.data_directory, self.metadata_directory, self.backup_directory]:
        ...
        self.production_files = {
            "loan_portfolio": {
                ...
                "required_columns": ["loan_id", "customer_id", "principal_amount", "interest_rate", 
                                   "origination_date", "maturity_date", "loan_status", "industry_code"],
                "business_rules": self._get_loan_validation_rules(),
                "critical": True
            },
            "payment_schedule": {
                "filename": "payment_schedule.csv", 
                "required_columns": ["payment_id", "loan_id", "due_date", "principal_amount", 
                                   "interest_amount", "total_amount", "payment_status"],
                "business_rules": self._get_payment_validation_rules(),
                "critical": True
            },
            "historic_payments": {
                "filename": "historic_real_payment.csv",
                "required_columns": ["loan_id", "payment_date", "amount_paid", "days_past_due",
                                   "payment_method", "collection_status"],
                "business_rules": self._get_historic_payment_rules(),
                "critical": True
            },
            "customer_master": {
                "filename": "customer_data.csv",
                "required_columns": ["customer_id", "customer_name", "credit_score", "industry_code",
                                   "registration_date", "risk_grade", "total_exposure"],
                "business_rules": self._get_customer_validation_rules(),
                "critical": True
            },
            "collateral_register": {
                "filename": "collateral_data.csv",
                "required_columns": ["collateral_id", "loan_id", "collateral_type", "estimated_value",
                                   "appraisal_date", "lien_position"],
                "business_rules": self._get_collateral_validation_rules(),
                "critical": False
            }
        }
    
    def synchronize_production_data(self, force_refresh: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not download_status["success"]:
                sync_results["critical_issues"].append("Failed to download production data")
                return sync_results
            ...
            for dataset_key, file_config in self.production_files.items():
                ...
                if not file_path.exists():
                    sync_results["critical_issues"].append(f"Missing critical file: {file_config['filename']}")
                    if file_config["critical"]:
                        all_files_valid = False
                    continue
                
                # Load and validate dataset
                validation_results = self._validate_production_dataset(file_path, file_config)
                ...
                sync_results["quality_assessment"][dataset_key] = asdict(quality_metrics)
                
                # Business rule validation
                business_validation = self._validate_business_rules(file_path, file_config)
                sync_results["business_validation"][dataset_key] = business_validation
                
                # Check for critical quality issues
                if quality_metrics.overall_quality in [DataQualityLevel.POOR, DataQualityLevel.CRITICAL]:
                    sync_results["critical_issues"].extend(quality_metrics.issues_identified)
                    if file_config["critical"]:
                        all_files_valid = False
            ...
            sync_results["sync_successful"] = all_files_valid and len(sync_results["critical_issues"]) == 0
            
            if sync_results["sync_successful"]:
                logger.info("✅ Production data synchronization completed successfully")
            else:
                logger.error("❌ Production data synchronization failed with critical issues")
    ...
    def _validate_production_dataset(self, file_path: Path, config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results["missing_columns"] = list(required_columns - actual_columns)
            validation_results["extra_columns"] = list(actual_columns - required_columns)
            validation_results["schema_compliance"] = len(validation_results["missing_columns"]) == 0
            ...
            if "loan_id" in df.columns:
                validation_results["unique_loan_ids"] = df["loan_id"].nunique() == len(df)
            
            if "customer_id" in df.columns:
                validation_results["customer_id_format_valid"] = df["customer_id"].notna().all()
            
            validation_results["validation_passed"] = (
                validation_results["schema_compliance"] and
                validation_results["duplicate_records"] == 0 and
    ...
    def _assess_data_quality(self, file_path: Path, config: Dict) -> DataQualityMetrics:
        ...
        try:
            ...
            overall_score = (completeness + consistency + accuracy + timeliness + validity) / 5
            ...
            if accuracy < 90:
                recommendations.append("Review business rule compliance and data validation")
            if timeliness < 85:
    ...
    def _get_loan_validation_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "loan_status": {"allowed_values": ["active", "paid_off", "charged_off", "delinquent"]},
    ...
    def _get_payment_validation_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_amount": {"min": 0},
            "payment_status": {"allowed_values": ["pending", "paid", "overdue", "partial"]}
        }
    ...
    def _get_historic_payment_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "payment_method": {"allowed_values": ["ach", "wire", "check", "online", "cash"]}
        }
    ...
    def _get_collateral_validation_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "collateral_type": {"allowed_values": ["real_estate", "equipment", "inventory", "receivables", "cash"]},
            "lien_position": {"allowed_values": ["first", "second", "third", "subordinate"]}
        }