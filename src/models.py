...
class LookerClient:
    ...
    def _authenticate(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"Looker authentication failed: {e.response.text}"
            ) from e
        except Timeout:
            raise RuntimeError(
                "Looker authentication timed out. Check network or server status."
            )
        except requests.exceptions.RequestException as e:
            raise ConnectionError(
                f"Failed to authenticate with Looker API: {str(e)}"
            ) from e
    ...
    def get_dashboard(self, dashboard_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to fetch dashboard {dashboard_id}: {e.response.text}"
            ) from e
        except Timeout:
            raise RuntimeError(f"Request to fetch dashboard {dashboard_id} timed out")
        except ValueError as e:
            raise RuntimeError(
                f"Invalid JSON response for dashboard {dashboard_id}"
            ) from e
    ...
    def get_look(self, look_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to fetch look {look_id}: {e.response.text}"
            ) from e
        except Timeout:
