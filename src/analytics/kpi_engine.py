...
class CommercialLendingKPIEngine:
    ...
    def _calculate_outstanding_portfolio(self, datasets: Dict[str, pd.DataFrame]) -> KPIResult:
        ...
        if payment_schedule is not None:
            ...
            latest_balances = payment_schedule.groupby("loan_id")["remaining_balance"].last()
        ...
        target_value = self._get_target_value("outstanding_portfolio", 7800000)  # Default $7.8M
    ...
    def _calculate_weighted_apr(self, datasets: Dict[str, pd.DataFrame]) -> KPIResult:
        ...
        if len(active_loans) == 0:
            ...
            total_weighted = (active_loans["interest_rate"] * active_loans["principal_amount"]).sum()
    ...
    def _calculate_concentration_risk(self, datasets: Dict[str, pd.DataFrame]) -> KPIResult:
        ...
        top_client_exposure = customer_exposure.max() if len(customer_exposure) > 0 else 0
        concentration_rate = top_client_exposure / total_portfolio if total_portfolio > 0 else 0
        
        target_concentration = self._get_target_value("concentration_limit", 0.15)  # 15% limit
        ...
        return KPIResult(
            ...
            status=self._determine_status(concentration_rate, target_concentration, lower_is_better=True),
    ...
    def _calculate_active_clients(self, datasets: Dict[str, pd.DataFrame]) -> KPIResult:
        ...
        active_clients = loan_data[loan_data["loan_status"] == "active"]["customer_id"].nunique()
    ...
    def _calculate_collection_rate(self, datasets: Dict[str, pd.DataFrame]) -> KPIResult:
        ...
        collection_rate = monthly_collected / monthly_scheduled if monthly_scheduled > 0 else 0.0
        target_collection = self._get_target_value("collection_rate", 0.95)  # 95% target
    ...
    def _calculate_monthly_disbursements(self, datasets: Dict[str, pd.DataFrame]) -> KPIResult:
        ...
        target_disbursements = self._get_target_value("monthly_disbursements", 450000)  # $450K target
    ...
    def _load_and_validate_datasets(self) -> Dict[str, pd.DataFrame]:
        ...
        required_datasets = ["loan_portfolio", "payment_schedule", "historic_payments", "customer_data"]
        datasets = {}
    ...
    def _determine_status(self, current: float, target: float, tolerance: float = 0.05, 
    ...
    def _calculate_trend(self, current_value: float, kpi_name: str) -> str:
        ...
        historical_avg = self.calculation_cache.get(f"{kpi_name}_historical", current_value)
