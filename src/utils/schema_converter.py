...
class CommercialLendingSchemaConverter:
    ...
    def convert_pandas_schema(self, df: pd.DataFrame, dataset_name: str = "dataset") -> Dict[str, Any]:
        """
        ...
        for column, dtype in df.dtypes.items():
            ...
            property_def = {
                ...
                "description": self._generate_enhanced_description(column, dtype_str, df[column]),
                "commercial_lending": {
            ...
            elif json_type == 'number' and any(term in column.lower() for term in ['rate', 'percent']):
            ...
            schema["commercial_lending"]["field_categories"][column] = property_def["commercial_lending"]["field_category"]
    ...
    def _analyze_field_constraints(self, series: pd.Series, json_type: str) -> Dict[str, Any]:
        ...
        try:
            if json_type == 'number':
                ...
                if not string_data.empty:
                    ...
                    if string_data.nunique() < len(string_data) * 0.5:  # Less than 50% unique
                        # Could be enumerated values
                        constraints["enum"] = string_data.unique().tolist()[:10]  # Limit to 10
    ...
    def _generate_enhanced_description(self, column: str, dtype: str, series: pd.Series) -> str:
    ...
    def generate_pydantic_model(self, dataset_name: str, output_file: Optional[str] = None) -> str:
    ...
    def generate_all_models(self, output_dir: str) -> Dict[str, str]:
        ...
        for dataset_name in self.datasets:
            if self.datasets[dataset_name].get("exists", True):
                ...
                model_code = self.generate_pydantic_model(dataset_name, str(output_file))
...
def convert_dataframe_schema(df: pd.DataFrame, dataset_name: str = "dataset") -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--format", choices=["pydantic", "jsonschema", "openapi"], 
                       default="pydantic", help="Output format")
    ...
    if args.output:
        if args.dataset:
            ...
            model_code = converter.generate_pydantic_model(args.dataset, 
                                                         f"{args.output}/{args.dataset}.py")
