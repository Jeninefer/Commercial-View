...
class CommercialLendingRetryError(Exception):
    ...
    def __init__(self, message: str, attempts: int, total_duration: float, last_exception: Exception):
...
class RetryConfig:
    ...
    def __init__(
        ...
        self.jitter = jitter
        self.retriable_exceptions = retriable_exceptions or [ConnectionError, TimeoutError]
        ...
        if self.state == "open":
            if self._should_attempt_reset():
                ...
                raise CommercialLendingRetryError(
                    ...
                    0, 0, Exception("Circuit breaker open")
                )
...
class CommercialLendingRetry:
    ...
    def __init__(self, config: RetryConfig = None):
        ...
        self.operation_configs = {
            "loan_pricing": RetryConfig(max_attempts=3, base_delay=0.5, strategy=RetryStrategy.LINEAR),
            "risk_assessment": RetryConfig(max_attempts=5, base_delay=1.0, strategy=RetryStrategy.EXPONENTIAL),
            "dpd_calculation": RetryConfig(max_attempts=3, base_delay=0.2, strategy=RetryStrategy.FIXED),
            "kpi_generation": RetryConfig(max_attempts=4, base_delay=2.0, strategy=RetryStrategy.FIBONACCI),
            "data_export": RetryConfig(max_attempts=8, base_delay=5.0, max_delay=120.0),
            "regulatory_report": RetryConfig(max_attempts=10, base_delay=3.0, timeout=1800.0),
            "portfolio_analysis": RetryConfig(max_attempts=6, base_delay=2.0, strategy=RetryStrategy.EXPONENTIAL)
        }
    ...
    def is_retriable_exception(self, exception: Exception, config: RetryConfig) -> bool:
        ...
        retriable_patterns = [
            ...
            "overloaded", "busy", "throttle", "temporary"
        ]
    ...
    def execute_with_retry(self, func: Callable[..., T], config: RetryConfig, *args, **kwargs) -> T:
            ...
            try:
                ...
                if config.timeout and (time.time() - start_time) > config.timeout:
                    raise CommercialLendingRetryError(
                        ...
                        attempt - 1, time.time() - start_time, last_exception
                    )
                ...
                if attempt == config.max_attempts or not self.is_retriable_exception(exc, config):
                    logger.error(f"Operation failed after {attempt} attempts: {exc}")
                    raise CommercialLendingRetryError(
                        f"Operation failed after {attempt} attempts: {exc}",
                        attempt, time.time() - start_time, exc
                    )
        ...
        raise CommercialLendingRetryError(
            ...
            config.max_attempts, time.time() - start_time, last_exception
        )
...
def get_retry_stats() -> Dict[str, Any]:
    ...
    return {
        ...
        "last_failure": default_retry.circuit_breaker.last_failure_time.isoformat() 
                       if default_retry.circuit_breaker.last_failure_time else None,
