...
class ConfigLoader:
    ...
    def _set_dynamic_defaults(self):
        ...
        if self.config['parameters'].get('analysis_date') is None:
            self.config['parameters']['analysis_date'] = datetime.now().strftime('%Y-%m-%d')
...
def load_config(config_path: Optional[Path] = None, environment: str = None) -> ConfigLoader:
    """
...
if __name__ == "__main__":
    ...
    print(f"2026 Portfolio Target: ${config.get('targets_2026.portfolio.total_portfolio_size'):,}")
