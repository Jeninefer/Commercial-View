...
def _safe_mean(series: Optional[pd.Series], default: Optional[float] = None) -> Optional[float]:
...
def calculate_delinquency_metrics(
    ...
    dpd_threshold: int = DEFAULT_DPD_THRESHOLD_30
) -> Dict[str, Any]:
    """
...
def calculate_advanced_risk_metrics(loan_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    try:
        ...
        metrics['npl_ratio'] = (
            (npl_amount / total_outstanding * 100.0)
            if total_outstanding > 0 else 0.0
        )
...
def calculate_marketing_kpis(
    ...
    try:
        ...
        if isinstance(funnel_data, pd.DataFrame):
            ...
            fd = funnel_data.iloc[0].to_dict() if len(funnel_data) == 1 else funnel_data.sum().to_dict()
        else:
            fd = funnel_data
