...
class DataLoader:
    ...
    def _get_file_names(self) -> Dict[str, str]:
        ...
        return {
            ...
            "customer_data": os.getenv(
                "CUSTOMER_DATA_CSV", DEFAULT_FILE_NAMES["customer_data"]
            ),
            "historic_real_payment": os.getenv(
                "HISTORIC_PAYMENT_CSV", DEFAULT_FILE_NAMES["historic_real_payment"]
            ),
            "payment_schedule": os.getenv(
                "PAYMENT_SCHEDULE_CSV", DEFAULT_FILE_NAMES["payment_schedule"]
            ),
            "collateral": os.getenv("COLLATERAL_CSV", DEFAULT_FILE_NAMES["collateral"]),
            "financials": os.getenv(
                "FINANCIALS_XLSX", DEFAULT_FILE_NAMES["financials"]
            ),
        }
    ...
    def load_from_google_sheets(
        self, sheet_key: str, sheet_name: Optional[str] = None
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            logger.info(
                f"✅ Loaded data from Google Sheet {sheet_key} (rows={len(df)})"
            )
            return df
    ...
    def load_all_data(self) -> Dict[str, pd.DataFrame]:
        ...
        logger.info(
            f"✅ Loaded {loaded_count}/{len(self.file_names)} datasets successfully"
        )
    ...
    def load_payment_schedule(self) -> pd.DataFrame:
        ...
        if (
            "payment_schedule" not in self.datasets
            or self.datasets["payment_schedule"].empty
        ):
    ...
    def load_historic_real_payment(self) -> pd.DataFrame:
        ...
        if (
            "historic_real_payment" not in self.datasets
            or self.datasets["historic_real_payment"].empty
        ):
            self.datasets["historic_real_payment"] = self.load_dataset(
