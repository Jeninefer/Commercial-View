...
class PortfolioOptimizer:
    ...
    def optimize(self, portfolio_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'customer_id' in portfolio_df.columns and 'outstanding_balance' in portfolio_df.columns:
            concentration = (portfolio_df.groupby('customer_id')['outstanding_balance'].sum() / 
                           portfolio_df['outstanding_balance'].sum())
            
            high_concentration = concentration[concentration > self.optimization_rules['max_concentration']]
    ...
    def generate_alerts(self, portfolio_df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        if 'days_past_due' in portfolio_df.columns:
            high_dpd = portfolio_df[portfolio_df['days_past_due'] > self.optimization_rules['max_dpd_threshold']]
