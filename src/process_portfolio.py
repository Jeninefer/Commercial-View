...
from kpi_calculator import calculate_progress_percentage, compute_arr, update_snapshot
...
def load_config(config_dir: str) -> Dict[str, Any]:
    ...
    config_files = ["column_maps.yml", "pricing_config.yml", "dpd_policy.yml", "export_config.yml"]
...
def generate_sample_output(export_config: Dict[str, Any]) -> None:
    ...
    dpd_data = {
        ...
        "portfolio_summary": {"total_loans": 1000, "total_exposure": 50000000, "default_rate": 0.05},
        "dpd_analysis": {
...
def enhanced_kpi_analysis(configs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    results = {
        ...
        "tenor_mix": csv_processor.calculate_tenor_mix(csv_data.get("loan_data") or pd.DataFrame()),
        "npl_metrics": csv_processor.calculate_npl_metrics(csv_data.get("historic_real_payment") or pd.DataFrame()),
    }
...
def comprehensive_analysis(configs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    weighted_stats = loan_analytics.calculate_weighted_stats(sample_portfolio, metrics=["apr", "eir", "term"])

    # Weighted metrics calculation
    weighted_metrics = metrics_calc.calculate_weighted_metrics(
        sample_portfolio, metrics=["credit_score", "apr"], weight_col="outstanding_balance"
    )
